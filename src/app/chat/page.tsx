...
import { ChatMessage, ChatInput, EmptyChatState, SuggestionChips } from '@/components/chat';
...
import { useMapStore, fetchUserLocation, extractFlightRouteFromCards } from '@/stores/mapStore';
...
export default function ChatPage() {
    ...
    const { activePanel, isDatePickerOpen, openDatePicker, closeDatePicker } = useUIStore();
    const { setUserLocation, setLocationLoading, userLocation, clearFlightRoute, flightRoute } = useMapStore();
    const { isOpen: isPanelOpen, selectedFlight: panelSelectedFlight, closePanel } = usePanelStore();
    ...
    const handleDateConfirm = useCallback((departureDate: string, returnDate: string | null) => {
        ...
        <div className="flex flex-col h-screen bg-paper">
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <main
                        className={cn(
                            ...
                            showSpecialPanel ? (activePanel === 'flights' ? 'w-[35%] max-w-[500px]' : 'w-1/2') :
                        ...
                        <div className="flex-1 overflow-y-auto">
                            <div
                                className={cn(
                                    ...
                                    isPanelOpen ? 'max-w-full px-6' :
                                        // When special panel is open
                                        showSpecialPanel ? (activePanel === 'flights' ? 'px-4' : 'max-w-full px-6') :
                                                    ...
                                                    message.flightCards.length > 0 && (
                                                        <div className="mt-4">
                                                            <FlightCarousel
                                                                ...
                                                                onShowAll={() => handleShowAllFlights(message.flightCards!)}
                                                            />
                                                        </div>
                                                    )}
                                            </div>
                        ...
                        <div className="border-t border-border bg-paper">
                            ...
                            >
                                <ChatInput
                                    ...
                                    placeholder={activePanel === 'flights' ? "Ask about these flights..." : undefined}
                                />
                            </div>
                        </div>
                    </main>
            ...
            {(showSpecialPanel || isPanelOpen) && (
                <div className="lg:hidden fixed inset-x-0 bottom-0 z-40">
                    <MobileBottomSheet
                        title={
                            activePanel === 'flights' ? 'Search Flights' :
                                activePanel === 'saved-trips' ? 'Saved Trips' :
                                    isPanelOpen ? 'Flight Details' :
                                        itineraryData?.trip_title || 'Trip Planner'
                        }
                        icon={isPanelOpen ? <Plane className="w-4 h-4 text-accent" /> : <Map className="w-4 h-4 text-accent" />}
                    >
