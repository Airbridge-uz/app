...
export const useChatStore = create<ChatState & ChatActions>((set, get) => ({
	...
	sendMessage: async (content: string, userId?: number) => {
		...
		try {
				...
				switch (chunk.type) {
					...
					case "data_payload":
						if (chunk.data && typeof chunk.data === "object") {
							...
							console.log(
								"[ChatStore] data_payload received:",
								payload.type,
								payload,
							);

							if (
								payload.type === "flight_cards" &&
								...
								if (Array.isArray(payload.suggestions)) {
									...
									console.log(
										"[ChatStore] Suggestions from flight_cards:",
										receivedSuggestions,
									);
								}
								set({
									flightCards: receivedFlightCards,
								...
								receivedGrounding =
									(payload.grounding as ItineraryGrounding) || null;
								// Also check for suggestions in itinerary payload
								if (Array.isArray(payload.suggestions)) {
									receivedSuggestions = payload.suggestions as Suggestion[];
									console.log(
										"[ChatStore] Suggestions from itinerary:",
										receivedSuggestions,
									);
								}
								set({
								...
								if (Array.isArray(suggestionsData)) {
									...
									console.log(
										"[ChatStore] ✅ Standalone suggestions received:",
										receivedSuggestions,
									);
			...
			console.log(
				"[ChatStore] Finalizing message with suggestions:",
				receivedSuggestions,
			);
			set((state) => ({
				messages: state.messages.map((msg) =>
					msg.id === assistantMessage.id
						? {
								...
								isStreaming: false,
								flightCards:
									receivedFlightCards.length > 0
										? receivedFlightCards
										: undefined,
								thoughtProcess:
									receivedThoughtProcess.length > 0
										? receivedThoughtProcess
										: undefined,
