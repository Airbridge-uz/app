...
export async function fetchUserLocation(): Promise<{
	...
	try {
		...
		if (!response.ok) {
			console.warn(
				"[MapStore] Failed to fetch user location:",
				response.status,
			);
...
export function extractFlightRouteFromCards(
	...
	console.log(
		"[MapStore] extractFlightRouteFromCards called with:",
		flightCards?.length,
		"cards",
	);
	...
	if (destination.coordinates?.lat && destination.coordinates?.lon) {
		...
		console.log(
			"[MapStore] Looking up destination by code:",
			destination.code,
			"→",
			destCoords,
		);
	...
	if (origin?.coordinates?.lat && origin?.coordinates?.lon) {
		...
		console.log(
			"[MapStore] Looking up origin by code:",
			origin.code,
			"→",
			originCoords,
		);
...
function getAirportCoordinates(code?: string): Coordinates | null {
	...
	const airports: Record<string, Coordinates> = {
		...
		AUH: { latitude: 24.433, longitude: 54.6511 },
		...
		MUC: { latitude: 48.3537, longitude: 11.775 },
