...
export const usePanelStore = create<PanelState & PanelActions>((set, get) => ({
    ...
    goBack: () => {
        ...
        const viewOrder: PanelView[] = ['flight-detail', 'fare-selection', 'passenger-details', 'confirmation'];
...
export const selectTotalPrice = (state: PanelState) => {
    ...
    if (state.selectedFlight) {
        return {
            total: state.selectedFlight.price.total ?? state.selectedFlight.price.amount ?? 0,
