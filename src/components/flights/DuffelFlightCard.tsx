...
export function DuffelFlightCard({
	flight,
	onSelect,
	compact = false,
	isSelected = false,
	...
	const isCurrentlySelected =
		isSelected || selectedFlight?.offer_id === flight.offer_id;
	...
	const durationHours =
		outboundSlice?.duration?.hours ||
		Math.floor((flight.duration?.total_minutes || 0) / 60);
	const durationMinutes =
		outboundSlice?.duration?.minutes ||
		(flight.duration?.total_minutes || 0) % 60;
					...
					<div className="flex items-center gap-5">
						...
						<div className="flex-1 grid grid-cols-3 gap-4">
							...
							<div className="text-center">
								<div className="text-base font-semibold text-gray-900">
									{String(durationHours).padStart(2, "0")}h{" "}
			...
			>
				...
				<div className="flex-1 flex items-center gap-8">
					...
					<div className="min-w-[120px] text-right">
						...
						{getLayoverDisplay() && (
							<div className="text-sm text-gray-500">{getLayoverDisplay()}</div>
						)}
					</div>
				</div>
				...
				<div className="border-t border-gray-200 bg-gray-50 animate-[slideDown_200ms_ease-out]">
					{isLoadingDetails ? (
						...
						<div className="p-6 space-y-4">
											...
											<div key={seg.segment_id || segIdx}>
												...
												<div className="flex items-center justify-center gap-4 mt-2 text-xs text-gray-500">
													...
													<span className="capitalize">
														{seg.cabin_class?.toLowerCase() || "Economy"}
													</span>
												</div>

												{/* Layover indicator */}
												{segIdx < slice.segments.length - 1 &&
													slice.layovers[segIdx] && (
														<div className="my-3 flex items-center justify-center">
															<div className="bg-amber-50 text-amber-700 text-xs px-3 py-1.5 rounded-full">
																Layover:{" "}
																{slice.layovers[segIdx].duration_formatted} in{" "}
																{slice.layovers[segIdx].city} (
																{slice.layovers[segIdx].airport_code})
															</div>
														</div>
													)}
											</div>
							...
							{/* Expiration Warning */}
							{offerDetails.booking_info?.expires_soon &&
								!offerDetails.booking_info.is_expired && (
									<div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-50 px-4 py-2 rounded-lg">
										<AlertTriangle className="w-4 h-4" />
										<span>
											This offer expires in{" "}
											{offerDetails.booking_info.expires_in_minutes} minutes
										</span>
									</div>
								)}
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<div className="text-2xl font-bold text-gray-900">
											{formatPrice(
												offerDetails.price.total,
												offerDetails.price.currency,
											)}
										</div>
										<div className="text-xs text-gray-500 mt-1">
											Base:{" "}
											{formatPrice(
												offerDetails.price.base,
												offerDetails.price.currency,
											)}{" "}
											+ Tax:{" "}
											{formatPrice(
												offerDetails.price.tax,
												offerDetails.price.currency,
											)}
										</div>
									</div>
