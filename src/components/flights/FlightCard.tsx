...
export function FlightCard({ flight }: FlightCardProps) {
    const { airline, price, duration, stops, slices, label, operating_carrier } = flight;
    ...
    const showOperatingCarrier = operating_carrier && operating_carrier !== airline.name;
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex-1 px-4">
                                <div className="flex items-center justify-center gap-2 text-xs text-muted mb-1">
                                    ...
                                    <span className="font-medium">{formatDuration(slice.duration.total_minutes)}</span>
                                </div>
                                ...
                                <div className="text-center text-xs text-muted mt-1">
                                    ...
                                    {slice.layovers && slice.layovers.length > 0 && (
                                        <span> · {slice.layovers.map(l => l.airport_code).join(', ')}</span>
                                    )}
                                </div>
                            </div>
