...
export function FlightSearchPanel({ onSearchComplete }: FlightSearchPanelProps) {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/duffel-search/search`, {
                method: 'POST',
            ...
            const flights: FlightCard[] = (data.offers || []).map((offer: Record<string, unknown>, index: number) => {
                const price = offer.price as Record<string, unknown> | undefined;
                const owner = offer.owner as Record<string, unknown> | undefined;
                const totalAmount = parseFloat(String(offer.total_amount || price?.total || 0));
                ...
                return {
                    ...
                    price: {
                        total: totalAmount,
                        currency: String(offer.total_currency || price?.currency || 'USD'),
    ...
    return (
        <div className="w-full flex flex-col h-full bg-white animate-[slide-in-right_300ms_ease-out]">
            ...
            <div className="px-8 pt-8 pb-6">
                ...
                <h1 className="text-2xl font-semibold text-ink">We&apos;ve got you covered</h1>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-8">
                ...
                <div className="mb-6">
                    <label className="block text-sm font-medium text-ink mb-3">Journey type</label>
                    <div className="flex items-center gap-6">
                        {(['one-way', 'return', 'multi-city'] as JourneyType[]).map((type) => (
                            <label
                                key={type}
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                        <label className="block text-sm font-medium text-ink mb-2">Passengers</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-ink mb-2">Class</label>
                        <div className="relative">
                            <select
                                value={cabinClass}
                    ...
                    <div className="mt-8">
                        ...
                        <div className="space-y-3">
                            {searchResults.map((flight, idx) => (
                                <DuffelFlightCard key={flight.offer_id || idx} flight={flight} />
                            ))}
                        </div>
                    </div>
                )}
