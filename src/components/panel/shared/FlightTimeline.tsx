					...
					<div className="relative pb-4">
						...
						<div className="text-sm">
							...
							<span className="ml-3 text-gray-600">
								Depart from{" "}
								{slice.origin?.name || slice.origin?.code || "Origin"} (
								{slice.origin?.code || "---"})
							</span>
						</div>
					</div>
	...
	return (
		<div className={cn("space-y-0", className)}>
			{slice.segments.map((segment, segIdx) => {
				...
				const depAirport =
					segment?.departure?.airport_code || slice.origin?.code || "";
				const arrAirport =
					segment?.arrival?.airport_code || slice.destination?.code || "";
					...
					<div key={segment?.flight_number || segIdx}>
						...
						<div className="relative pl-8">
								...
								<div className="text-sm">
									...
									<span className="ml-3 text-gray-600">
										...
										{segIdx === slice.segments.length - 1
											? slice.destination?.name || arrAirport
											: arrAirport}{" "}
										({arrAirport})
									</span>
								</div>
							...
							<div className="flex flex-wrap gap-2 pb-4 text-xs text-gray-600">
								<span className="px-2 py-1 bg-gray-100 rounded">Economy</span>
								<span className="px-2 py-1 bg-gray-100 rounded">
									{segment?.airline?.name || airlineName || "Airline"}
								</span>
								{segment?.aircraft && (
									<span className="px-2 py-1 bg-gray-100 rounded">
										{segment.aircraft}
								...
								<span className="px-2 py-1 bg-gray-100 rounded inline-flex items-center gap-1">
									<Briefcase className="w-3 h-3" />1 carry-on bag
								</span>
								<span className="px-2 py-1 bg-gray-100 rounded inline-flex items-center gap-1">
									<Luggage className="w-3 h-3" />1 checked bag
								</span>
							</div>
						</div>
						...
						{segIdx < slice.segments.length - 1 && slice.layovers?.[segIdx] && (
							<div className="my-4 mx-4 px-4 py-3 bg-amber-50 rounded-lg text-sm text-amber-800 border border-amber-100">
								{formatDuration(slice.layovers[segIdx]?.duration_minutes)}{" "}
								layover at {slice.layovers[segIdx]?.city || "Airport"} (
