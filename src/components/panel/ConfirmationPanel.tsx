...
export function ConfirmationPanel() {
	...
	const { bookingResult, selectedFlight, resetPanel, closePanel } =
		usePanelStore();
	...
	const originCode =
		postBooking?.booking_summary.origin_code ||
		outboundSlice?.origin.code ||
		"XXX";
	const destCode =
		postBooking?.booking_summary.destination_code ||
		outboundSlice?.destination.code ||
		"XXX";
	const originCity =
		postBooking?.booking_summary.origin_city ||
		outboundSlice?.origin.city ||
		originCode;
	const destCity =
		postBooking?.booking_summary.destination_city ||
		outboundSlice?.destination.city ||
		destCode;
	const departureDate =
		postBooking?.booking_summary.departure_date ||
		outboundSlice?.departure.date ||
		"";
					...
					<div className="space-y-0 divide-y divide-gray-100">
						<div className="flex justify-between py-2.5">
							...
							<span className="text-sm font-medium text-gray-900">
								{postBooking?.booking_summary.passengers_count ||
									bookingResult.passengers.length}
							</span>
						</div>
