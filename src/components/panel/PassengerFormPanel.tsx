...
export function PassengerFormPanel() {
	...
	const price =
		selectedFare?.price.total ??
		selectedFlight?.price?.total ??
		selectedFlight?.price?.amount ??
		0;
	const currency =
		selectedFare?.price.currency ?? selectedFlight?.price?.currency ?? "GBP";
	...
	return (
		<div className="flex flex-col h-full">
			...
			<PriceBar
				...
				currency={currency}
				buttonText={
					paymentTiming === "pay_now" ? "Complete Booking" : "Hold Order"
				}
		...
		<div className="mb-6 last:mb-0">
			...
			<div className="space-y-4">
				...
				<div className="grid grid-cols-3 gap-4">
					...
					<FormField label="Given name" required error={getError("given_name")}>
						<input
							type="text"
					...
					<FormField label="Gender" required>
						<div className="relative">
							<select
								...
								onChange={(e) =>
									onUpdate({ gender: e.target.value as "male" | "female" })
								}
								className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-gray-900"
							>
