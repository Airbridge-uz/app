...
export function TravelDatePicker({
	...
	const handleConfirm = () => {
		...
		const retStr =
			returnDate && !isOneWay ? format(returnDate, "yyyy-MM-dd") : null;
	...
	const renderMonth = (monthDate: Date, isSecond: boolean = false) => {
		...
		return (
			<div
				className={cn("flex-1", isSecond && "border-l border-gray-100 pl-6")}
			>
				{/* Month Header */}
				...
				<div className="grid grid-cols-7 gap-1">
					...
					{days.map((day) => {
						...
						const isSelectedDeparture = !!(
							departureDate && isSameDay(day, departureDate)
						);
						...
						if (!isOneWay && departureDate) {
							if (returnDate) {
								...
								hoverDate &&
								isAfter(hoverDate, departureDate) &&
								selectingReturn
							) {
								isInRange = isWithinInterval(day, {
									start: departureDate,
							...
							<div key={day.toISOString()} className="relative">
								...
								<button
									...
									className={cn(
										...
										isSelected &&
											"bg-amber-500 text-white rounded-full font-medium shadow-sm",
