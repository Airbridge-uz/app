            ...
            <div className="w-64 p-4">
                ...
                <div className="grid grid-cols-7 gap-1">
                    ...
                    {days.map(day => {
                        ...
                        let isInRange = false;
                        if (startObj && endObj && isWithinInterval(day, { start: startObj, end: endObj })) {
                            isInRange = true;
                        } else if (startObj && !endObj && hoverDate && isAfter(hoverDate, startObj) && isWithinInterval(day, { start: startObj, end: hoverDate })) {
                            isInRange = true;
                        }
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    ...
                                    !isDisabled && "hover:bg-accent/10 hover:text-accent text-ink",
                                    (isSelectedStart || isSelectedEnd) && "bg-accent text-white hover:bg-accent hover:text-white",
                                    isInRange && !isSelectedStart && !isSelectedEnd && "bg-accent-muted/50 rounded-none text-ink width-[calc(100%+4px)] -mx-[2px]"
                                )}
                            >
                                {format(day, 'd')}
                            </button>
    ...
    return (
        <div className="relative" ref={containerRef}>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-ink mb-2">Departure date</label>
                    <div
                    ...
                    >
                        ...
                        {startDate ? format(new Date(startDate), 'dd MMM yyyy') : 'DD / MM / YYYY'}
                    </div>
                </div>
                <div>
                    <label className="block text-sm font-medium text-ink mb-2">Return date</label>
                    <div
                    ...
                    >
                        <CalendarIcon className="w-4 h-4 text-muted" />
                        {endDate ? format(new Date(endDate), 'dd MMM yyyy') : 'DD / MM / YYYY'}
                    </div>
                </div>
            </div>
